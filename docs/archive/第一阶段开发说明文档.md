...
interface Item {
  ...
  unlocked?: boolean;            // 是否已解锁（通过UserProgressService管理）
...
class ItemService {
  ...
  // 检查物品是否解锁（委托给UserProgressService）
...
### 4. 用户进度服务 (UserProgressService) ✨新增
...
#### 新增功能
1. **UserProgressService** - 用户进度管理服务
   - 管理物品解锁状态
   - localStorage 持久化存储
   - 支持批量解锁操作
...
#### 技术改进
- 使用 Set 数据结构优化解锁状态查询性能
- 修复 MUI v7 Grid 组件兼容性问题
