...
// 汇总所有物品
export const allItems: Item[] = [...resources, ...materials, ...components]
...
// 汇总所有配方
export const allRecipes: Recipe[] = [...smeltingRecipes, ...craftingRecipes]

// 创建配方ID映射
export const recipesById: Record<string, Recipe> = allRecipes.reduce(
  (acc, recipe) => {
...
export const getRecipesForItem = (itemId: string): Recipe[] => {
  return allRecipes.filter((recipe) => recipe.products.some((product) => product.itemId === itemId))
}
