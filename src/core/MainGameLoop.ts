...
export class MainGameLoop {
  ...
  private updateFacilityProduction(
    ...
    const { updateFacility, batchUpdateInventory, getInventoryItem, trackCraftedItem, trackMinedEntity } =
      gameStoreActions;
  ...
  private completeManualCraft(task: CraftingTask, recipe: Recipe | null): void {
    ...
    // 对于手动采集任务（recipeId 以 manual_ 开头），统一通过 completeCraftingTask 处理库存
