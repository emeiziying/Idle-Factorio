...
describe('RecipeService 配方服务测试', () => {
  ...
  beforeEach(async () => {
    ...
    mockDataService = {
      getItem: vi.fn((itemId) => dataService.getItem(itemId) || {
        id: itemId,
        name: itemId,
        category: 'test',
        stack: 100,
      ...
      getRecipe: vi.fn((recipeId) => 
        testRecipes.find(r => r.id === recipeId)
      ),
  ...
  describe('getRecipeRecommendations 配方推荐测试', () => {
    it('应按效率推荐配方', () => {
      // 获取铁板的效率推荐
      const recommendations = RecipeService.getRecipeRecommendations('iron-plate', ['stone-furnace', 'steel-furnace'], 'efficiency')
