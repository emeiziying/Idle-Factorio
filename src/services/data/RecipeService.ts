...
export class RecipeService {
  ...
  static searchRecipes(query: string): Recipe[] {
    ...
    return this.allRecipes.filter(
      (recipe) => recipe.name.toLowerCase().includes(lowerQuery) || recipe.id.toLowerCase().includes(lowerQuery)
    );
  }
  ...
  static getEnhancedRecipeStats(
    ...
    const averageEfficiency =
      efficiencies.length > 0 ? efficiencies.reduce((sum, eff) => sum + eff, 0) / efficiencies.length : 0;
