...
describe('RecipeService 配方服务测试', () => {
  ...
  describe('getRecyclingRecipes 获取回收配方测试', () => {
    ...
    it('应根据itemId过滤回收配方', () => {
      ...
      // 验证包含铁板回收配方
      expect(
        ironPlateRecycling.some(r => r.id.includes('iron-plate') && r.flags?.includes('recycling'))
      ).toBe(true);
    });
  });
  ...
  describe('getOptimalProductionPath 最优生产路径测试', () => {
    it('应返回最优生产路径', () => {
      // 获取铁齿轮的最优生产路径
      const productionPath = RecipeService.getOptimalProductionPath('iron-gear-wheel', 10, [
  ...
  describe('getRecipeRecommendations 配方推荐测试', () => {
    it('应按效率推荐配方', () => {
      ...
      if (recommendations.length > 1) {
        ...
        const secondEfficiency = RecipeService.getRecipeEfficiency(
          recommendations[1],
          'iron-plate'
        );
