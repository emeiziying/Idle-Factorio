...
export class PowerService {
  ...
  // 发电设施ID
  private readonly POWER_GENERATORS = ['steam-engine', 'steam-turbine', 'solar-panel', 'accumulator'];

  // 发电设施类型标识
  private readonly GENERATOR_TYPES = ['steam-engine', 'steam-turbine', 'solar-panel', 'accumulator'];
  ...
  calculatePowerBalance(facilities: FacilityInstance[]): PowerBalance {
    ...
    // 计算满足率
    const satisfactionRatio = generation.capacity > 0 ? Math.min(1, generation.capacity / consumption.demand) : 0;
   ...
   * 获取电力优先级建议
   */
  getPowerPriorityRecommendations(_facilities: FacilityInstance[], powerBalance: PowerBalance): string[] {
