...
export class FuelService {
   ...
   * 添加燃料到缓存区
   */
  addFuel(buffer: GenericFuelBuffer, itemId: string, quantity: number, facilityId: string): AddFuelResult {
    if (!buffer) {
      return { success: false, reason: 'No fuel buffer' };
    }
  ...
  smartFuelDistribution(
    ...
    const needsFuel = facilities
      .filter((f) => {
        ...
        return f.fuelBuffer && f.status !== FacilityStatus.STOPPED && itemData?.machine?.type === 'burner';
      })
      .sort((a, b) => {
