...
export class PowerService {
  ...
  calculatePowerBalance(facilities: FacilityInstance[]): PowerBalance {
    ...
    const satisfactionRatio =
      generation.capacity > 0 ? Math.min(1, generation.capacity / consumption.demand) : 0;
    ...
    if (generation.capacity > consumption.demand * (constants.power.surplusThreshold / 100)) {
      ...
      generation.capacity >=
      consumption.demand * (constants.power.balancedThreshold / 100)
    ) {
      status = 'balanced';
    } else {
      status = 'deficit';
  ...
  getPowerPriorityRecommendations(
    ...
    if (powerBalance.satisfactionRatio < 0.5) {
      recommendations.push(
        '电力严重不足，生产效率仅为 ' + (powerBalance.satisfactionRatio * 100).toFixed(0) + '%'
      );
    }
