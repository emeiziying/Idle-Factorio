...
export class TechnologyService extends BaseService {
  ...
  constructor() {
    super();
    this.serviceName = 'TechnologyService';
    this.dependencies = ['DataService', 'UserProgressService'];
  ...
  async initialize(): Promise<void> {
    try {
      // 等待依赖服务
      this.userProgressService = ServiceLocator.get<UserProgressService>('UserProgressService');
  ...
  private convertTechnologyFromDataJson(techId: string, techRecipe: TechRecipe, techItem: TechItem): Technology {
    ...
    let localizedName = this.dataService!.getLocalizedRecipeName(techId);
    if (!localizedName || localizedName === techId) {
      localizedName = this.dataService!.getLocalizedItemName(techId);
    }
    if (!localizedName || localizedName === techId) {
  ...
  private async loadTechnologiesFromDataJson(): Promise<void> {
    return this.safeAsync(async () => {
      ...
      await this.dataService!.loadGameData(); // 确保游戏数据已加载
      await this.dataService!.loadI18nData('zh'); // 确保国际化数据已加载
      
      // 获取原始游戏数据
      const gameData = this.dataService!.getRawGameData() as { recipes: unknown[]; items: unknown[] };
  ...
  private async loadTechCategoriesFromDataJson(): Promise<TechCategory[]> {
    return this.safeAsync(async () => {
      ...
      const categories = this.dataService!.getTechCategories();
      ...
      const techCategories: TechCategory[] = categories.map((cat: unknown, index: number) => {
        ...
        const localizedName = this.dataService!.getLocalizedCategoryName(categoryData.id) || categoryData.name;
    ...
    }, 'loadTechCategoriesFromDataJson', []);
  }
  ...
  public async startResearch(techId: string): Promise<ResearchResult> {
    ...
    }, 'startResearch', { success: false, error: 'Failed to start research' });
  }
  ...
  public addToResearchQueue(techId: string, priority: ResearchPriority = ResearchPriority.NORMAL): QueueResult {
    ...
    }, 'addToResearchQueue', { success: false, error: 'Failed to add to queue' });
  }
  ...
  getServiceInfo() {
    return {
      ...
      storageKeys: ['tech_state', 'tech_unlocks']
    };
  }

  async healthCheck() {
    const isHealthy = this.isInitialized && this.techTree.size > 0;
    return {
      healthy: isHealthy,
      message: isHealthy ? 'Service is running' : 'Service not initialized'
    };
  }
}
