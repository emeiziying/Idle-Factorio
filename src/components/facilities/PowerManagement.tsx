      ...
      <Card sx={{ mb: 2 }}>
        <CardContent>
          <Box display="flex" alignItems="center" gap={1} mb={2}>
            ...
            <Chip
              label={powerBalance.status === 'surplus' ? '盈余' : 
                     powerBalance.status === 'balanced' ? '平衡' : '不足'}
          ...
          <Box mb={1}>
            ...
            <LinearProgress
              ...
              color={powerBalance.satisfactionRatio >= 0.95 ? 'success' : 
                     powerBalance.satisfactionRatio >= 0.7 ? 'warning' : 'error'}
      ...
      <Card sx={{ mb: 2 }}>
        <CardContent>
                  ...
                  <ListItemText
                    primary={
                      <Box display="flex" alignItems="center" gap={1}>
                        <Typography>
                          {generatorId === 'steam-engine' ? '蒸汽机' :
                           generatorId === 'steam-turbine' ? '汽轮机' : '太阳能板'}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
          ...
          {(generatorStats.get('steam-engine')?.count || 0) + 
           (generatorStats.get('steam-turbine')?.count || 0) > 0 && (
            ...
            {Object.entries(powerBalance.consumptionByCategory).map(([category, power]) => (
              <Grid item xs={6} sm={4} key={category}>
                <Box textAlign="center">
                  <Typography variant="body2" color="text.secondary">
                    {category === 'mining' ? '采矿' :
                     category === 'smelting' ? '熔炼' :
                     category === 'crafting' ? '制造' :
                     category === 'chemical' ? '化工' :
