    ...
    <Box>
      ...
      <Card sx={{ mb: 2 }}>
        <CardContent>
          <Box display="flex" alignItems="center" gap={1} mb={2}>
            ...
            <Chip
              label={powerBalance.status === 'surplus' ? '盈余' : powerBalance.status === 'balanced' ? '平衡' : '不足'}
          ...
          <Box mb={1}>
            <Box display="flex" justifyContent="space-between" mb={0.5}>
              ...
              <Typography variant="body2">{(powerBalance.satisfactionRatio * 100).toFixed(0)}%</Typography>
            </Box>
            <LinearProgress
              variant="determinate"
      ...
      <Card sx={{ mb: 2 }}>
        <CardContent>
              ...
              return (
                <ListItem key={generatorId}>
                  ...
                  <Box display="flex" alignItems="center">
                    ...
                    <Typography sx={{ mx: 1, minWidth: 30, textAlign: 'center' }}>{stats.count}</Typography>
                    <IconButton
                      size="small"
                      onClick={() => handleGeneratorChange(generatorId, 1)}
          ...
          {/* 蒸汽供应提示 */}
          {(generatorStats.get('steam-engine')?.count || 0) + (generatorStats.get('steam-turbine')?.count || 0) > 0 && (
