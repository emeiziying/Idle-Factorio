  ...
  ({
    ...
    const getPriorityLabel = React.useCallback((priority: number) => {
      switch (priority) {
        ...
        case 1:
          return { label: '普通', color: 'primary' as const };
        case 2:
        ...
        <CardContent sx={{ '&:last-child': { pb: 2 } }}>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {totalQueueTime > 0 && (
                <Chip size="small" label={formatTime(totalQueueTime)} color="info" variant="outlined" />
              )}
            </Box>
                  ...
                  {(() => {
                    ...
                    return (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        <FactorioIcon itemId={tech?.icon || currentResearch.techId} size={32} showBorder={false} />
                        ...
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            ...
                            {currentResearch.timeRemaining && <> - 剩余 {formatTime(currentResearch.timeRemaining)}</>}
                          </Typography>
                        </Box>
                      </Box>
                    );
          ...
          <Collapse in={expanded}>
            {queue.length === 0 ? (
              ...
              >
                ...
                <Typography variant="caption">点击科技节点的 "添加到队列" 按钮来添加研究任务</Typography>
              </Box>
            ) : (
              <List dense>
                {queue.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <ListItemIcon>
                        <FactorioIcon itemId={tech?.icon || item.techId} size={24} showBorder={false} />
                      </ListItemIcon>
                      ...
                      <ListItemText
                        primary={
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            {isBlocked && <Chip size="small" label="等待前置" color="warning" variant="outlined" />}
                          </Box>
                        }
                        secondary={
                          <>
                      ...
                      <ListItemSecondaryAction>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          ...
                          <IconButton size="small" onClick={() => onRemoveFromQueue?.(item.techId)} color="error">
                            <RemoveIcon />
                          </IconButton>
                        </Box>
                      </ListItemSecondaryAction>
          ...
          {queue.length > 0 && (
            <Box sx={{ mt: 2, pt: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Typography variant="body2">自动研究</Typography>
              ...
              <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                {autoResearch ? '队列中的科技将按顺序自动开始研究' : '需要手动点击开始按钮来进行研究'}
              </Typography>
            </Box>
          )}
        </CardContent>
