...
import {
  tickFacilityFuelState,
  tickFacilityProductionState,
  updateFacilityPowerState,
} from '@/engine/core/facilitySystems';
...
export const tickGame = (state: GameState, context: TickContext): TickResult => {
  ...
  const productionResult = tickFacilityProductionState(
    fuelResult.state,
    context.catalog,
    normalizedDeltaMs
  );
  const researchResult = tickResearch(productionResult.state, context.catalog, normalizedDeltaMs);
