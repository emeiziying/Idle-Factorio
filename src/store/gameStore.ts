...
const useGameStore = create<GameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    completeCraftingTask: (taskId: string) => {
      ...
      if (task.isIntermediateProduct && task.chainId) {
        ...
        if (chain) {
          ...
          const completedTasks =
            chain.tasks.filter((t) => state.craftingQueue.find((qt) => qt.id === t.id)?.status === 'completed').length +
            1; // +1 for the current task
          const totalProgress = completedTasks / chain.tasks.length;

          set((state) => ({
            craftingChains: state.craftingChains.map((c) => (c.id === task.chainId ? { ...c, totalProgress } : c)),
          }));
      ...
      // 先标记任务为已完成，然后移除
      set((state) => ({
        craftingQueue: state.craftingQueue.map((t) => (t.id === taskId ? { ...t, status: 'completed' as const } : t)),
      }));
    ...
    checkResearchTriggers: async () => {
      ...
      try {
        ...
        const techRecipes = allRecipes.filter(
          (recipe) => recipe.category === 'technology' && recipe.researchTrigger && !state.unlockedTechs.has(recipe.id)
        );
    ...
    saveGame: () => {
      ...
      const state = get();
      getGameStorageService()
        .then((gameStorageService) => {
