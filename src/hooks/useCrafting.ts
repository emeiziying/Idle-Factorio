...
import { DependencyService, type CraftingChainAnalysis, type CraftingDependency } from '../services/crafting/DependencyService';
...
export const useCrafting = () => {
  ...
  const executeChainCrafting = (chainAnalysis: CraftingChainAnalysis) => {
    ...
    const chainTasks = chainAnalysis.tasks.map((task, index) => {
      ...
      console.log(
        `[链式任务创建] 任务${index}: ${task.itemId} x${task.quantity}, 中间产物: ${isIntermediate}, ID: ${task.id}`
      );
      return {
        ...task,
        // 标记中间产物（除了最后一个任务）
