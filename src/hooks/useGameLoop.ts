...
import { completeCrafting, startCrafting, updateProgress } from '../store/slices/craftingSlice'
      ...
      if (activeCrafting) {
        ...
        if (recipe) {
          ...
          if (remainingItems > 0) {
            ...
            // 总进度 = (已完成数量 + 当前进度) / 总数量
            const totalProgress =
              ((completedItems + currentItemProgress / 100) / activeCrafting.quantity) * 100
